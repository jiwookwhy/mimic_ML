...
path = "../processed_data/processedFrames_04172022.pkl"

with open(path,'rb') as f: filteredOutputFrames = pickle.load(f)
filteredHadms = list(filteredOutputFrames.keys())
...
#commenting these out b/c they are otherwise NOT used in rest of script!!!
...
#removed clusterEthnicities -- this is already done in dataprep script.
#and we want final vals to be actual #'s NOT STRING!!!
...
def getSamples(outputFrames, i):
    ...
    for t in range(len(cleanOutput) - 1):
        ...
        for vital in ['Heart Rate', 'Respiratory Rate', 'Non Invasive Blood Pressure systolic',
            ...
            change = abs(currval - nextval)
            if change > (0.2*currval):
        ...
        if (extubating or stayingOff):
            for vital in regulateVitals.keys():
                ...
                if ((math.isnan(minv) != 1) and value < minv):
                    reward -= 1
                if ((math.isnan(maxv) != 1) and value > maxv):
                    reward -= 1
        ...
        # added firstICU col here too -- this is NOT supposed to be a feature. It's just an ID!
        unneeded_cols = ['timestamp', 'hadm', 'subject', 'firstICU', 'Admdays', 'Vented', 'SBT']
        currState = currState.drop(columns=unneeded_cols)
        nextState = nextState.drop(columns=unneeded_cols)
...
def produceSamples(samples, h):
    #took out try/except statement here!
...
def main():
    ...
    with open("../pickles/allSamples_04172022.pkl",'wb') as f:
    ...
    print('These are the number of hadms with good samples:{}'.format(len(hadms)))
    print('This is number of total original samples: {}'.format(len(allSamples)))
    print('This is number of good/kept samples: {}'.format(len(goodInds)))
    ...
    #put currStates and nextStates into pd df and then to numpy...
    #this is very likely redundant -- and we can just do everyhting in np
    #just tried to maintain original struct of code as much as possible
    #after removing feat collapse for gender, ethnicity and admittype (which is already done in previous script!)
    currStates = (pd.DataFrame(np.concatenate(currStates))).to_numpy()
    nextStates = (pd.DataFrame(np.concatenate(nextStates))).to_numpy()
    ...
    with open("../pickles/preppedSamples_04172022.pkl",'wb') as f:
        pickle.dump((currStates, nextStates, actions, rewards, discretizedActions, discretizedActionSum), f)
